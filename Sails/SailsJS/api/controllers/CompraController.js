...
module.exports = {
  ...
  /** 
   *  Se agrega validacion en misOrdenes para consultar ordenes de clientes en el perfil de Administrador!!!
   *
   */
  ...
  misOrdenes: async (peticion, respuesta) => {
    ...
    else if (peticion.session.admin) {   /* validando ordenes de clientes con credenciales Admin */
      console.log(`consulta mis ordenes con perfil admin:  ${JSON.stringify(peticion.query.cliente)}`);
      let ordenes = await Orden.find({ cliente: peticion.query.cliente }).sort('id desc')
      respuesta.view('pages/mis_ordenes', { ordenes })
    }
    else if (peticion.session.cliente) {
      console.log(`consulta mis ordenes con perfil cliente`);
  ...
  ordenDeCompra: async (peticion, respuesta) => {
    if (peticion.session.admin || peticion.session.cliente) {
